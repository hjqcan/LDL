...
{
    public partial class UCStockGoodsProvide : DevExpress.XtraEditors.XtraUserControl
    {

        #region 属性

        List<StockGoodsProvide> dataSource;
        List<CoSite> lstSite;
        Dictionary<string, object> query;

        #endregion

        public UCStockGoodsProvide()
        {
            InitializeComponent();
        }

        private void UCStockGoodsProvide_Load(object sender, EventArgs e)
        {
            BindParameter();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (validationProvider.Validate())
                {
                    ...
                    WcfServiceHelper.Client.InsertStockGoodsProvide(provide);
        ...
        {
            if (gvGoods.FocusedRowHandle < 0)
            {
                ControlsHelper.ShowWarningMessage("选择发放物料", "请从左侧表格中选择发放的物料");
                return;
            }
            ...
            StockGoods objGoods = gvGoods.GetRow(gvGoods.FocusedRowHandle) as StockGoods;
            lookUpGoods.EditValue = objGoods.Goods;
            if (objGoods.Price.HasValue)
                spinEditPrice.Value = objGoods.Price.Value;

            groupHeader.Tag = "NEW|" + objGoods.OwnerSite;
            groupHeader.Text = "新增";
        ...
        void _barbtnDelete_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            {
                StockGoodsProvide provide = (StockGoodsProvide)gvData.GetRow(gvData.FocusedRowHandle);
                ...
                if (ControlsHelper.ShowConfirmMessage("确认", string.Format("确定要删除记录吗？")) == DialogResult.Yes)
                {
                    ...
                    {
                        this.Cursor = Cursors.WaitCursor;
                        WcfServiceHelper.Client.DeleteStockGoodsProvide(provide.ID);
                        BindList();
        ...
        void BindList()
        {
            ...
            dataSource = WcfServiceHelper.Client.GetStockGoodsProvideBySite(query[btneditSite.Name].ToString()).ToList();
            gridData.DataSource = dataSource;
            gvData.BestFitColumns();

            gridGoods.DataSource = WcfServiceHelper.Client.GetStockGoodsBySite(query[btneditSite.Name].ToString()).ToList();
            gvGoods.BestFitColumns();
        }
        ...
        void BindParameter()
        {
            ...
            (gvGoods.Columns["OwnerSite"].ColumnEdit as DevExpress.XtraEditors.Repository.RepositoryItemLookUpEdit).DataSource = lstSite;
            //物料品名
            ...
            (gvGoods.Columns["Goods"].ColumnEdit as DevExpress.XtraEditors.Repository.RepositoryItemLookUpEdit).DataSource = FrequentlyUsedData.GetParameterList("StockGoods", false);
            //领取人
